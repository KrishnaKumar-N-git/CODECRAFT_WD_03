/**
 * SMK Motor Spare Parts — Admin API System
...
const API = {
  ...
  async request(path, options = {}) {
    ...
    try {
      ...
      console.warn(`⚠️ Admin API Call failed for ${path}, using static mode fallback:`, err.message);
  ...
  handleFallback(path, options = {}) {
    ...
    // Enquiries / Support GET /support
    if (cleanPath === '/support') {
      return Promise.resolve({ enquiries: [], total: 0 });
    }

    // Auth Login Fallback for preview mode
    if (cleanPath === '/auth/login') {
      const adminUser = { id: 'admin_1', username: 'admin', role: 'admin', name: 'SMK Admin' };
      this.setToken('mock_admin_token');
      localStorage.setItem('user', JSON.stringify(adminUser));
      return Promise.resolve({ user: adminUser, token: 'mock_admin_token' });
    }

    return Promise.resolve({ success: true, message: 'Admin Static Mode Fallback' });
  },
  ...
  loginOtpVerify(emailOrPhone, code) { return this.request('/auth/login-otp/verify', { method: 'POST', body: { phone: emailOrPhone, code } }); },
  ...
  // Enquiries / Support
  getEnquiries(params = {}) { const qs = new URLSearchParams(params).toString(); return this.request(`/support?${qs}`); },
  updateEnquiryStatus(id, status, adminNotes = '') { return this.request(`/support/${id}`, { method: 'PUT', body: { status, adminNotes } }); },
  deleteEnquiry(id) { return this.request(`/support/${id}`, { method: 'DELETE' }); },
};
